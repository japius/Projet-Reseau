...
struct list *init_list_entry(struct neighbor *peer,int times_sent){
...
short add_neighbor_to_flood(struct data_index *index,struct neighbor *peer){
	struct flood_entry *flood=get_flood(data_index);
	if(flood==NULL){
		return 0;
	}
	struct list_entry l;
...
short add_entry(struct data_index *data,char *msg,struct list *entry){
	if(entry){
		...
		DATAF->last->next=init_flood_entry(data,msg,entry);
		if(DATAF->last->next==NULL) return 0;
		DATAF->last=last->next;
...
short remove_neighbor_from_flood(struct data_index *data,struct neighbor *peer){
	...
	return remove_element(&entry,peer,&compare_n);
}


//Fonction auxialiare pour supprimer une donnée à inonder et sa liste de voisins
short rm_entry_aux(struct  list *curr,struct flood_entry *entry){
	if(curr->next){
...
short rm_entry(struct data_index *key){
	if(DATAF){
		if(DATAF->first){
			if(compare_d(flood->first->index,entry->index)==0){
				struct list *tmp=flood->first;
...
struct flood_entry *get_flood(struct data_index *data){
	return (flood_entry *)get_element(DATAF,data,&compare_n);
}
