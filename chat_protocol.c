...
//Pour mettre des fichiers
static int
file_upload_cb(void *data, const char *name, const char *filename,char *buf, int len, enum lws_spa_fileupload_states state){
	struct form_data *fdata =(struct form_data *)data;
	#if !defined(LWS_WITH_ESP32)
		int n;
	...
	#endif
	switch (state) {
		//un nouveau fichier commence à arriver
		case LWS_UFS_OPEN:
			lws_strncpy(fdata->filename, filename, sizeof(fdata->filename));
	...
	#if !defined(LWS_WITH_ESP32)
			//??
			fdata->fd = (lws_filefd_type)(long long)lws_open("/tmp/post-file",
		...
		case LWS_UFS_CONTENT:
			if (len) {
				fdata->file_length += len;

				/* if the file length is too big, drop it */
				if (file->file_length > 100000)
					return 1;
...
static int
format_result(struct form_data *pss)
{
...
callback_chat(struct lws *wsi, enum lws_callback_reasons reason,
		   void *user, void *in, size_t len)
{
	struct form_data *pss =
			(struct form_data *)user;
	unsigned char *p, *start, *end;
	int n;
	...
	case LWS_CALLBACK_RECEIVE:
		//normalement c'est si on recoit par ici,c'est le truc de quelqu'un qu'il faut juste utiliser
		//on a recu un data
		//unsigned char *buf=(unsigned char*)malloc(LWS_PRE)
		break;
